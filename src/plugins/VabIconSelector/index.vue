<script lang="ts" setup>
  ...
  const fetchData: any = () => {
    ...
    // 根据state.queryForm.title过滤数据
    const listData = baseIcon.filter((item: any) =>
      item.includes(state.queryForm.title)
    )

    // 根据state.queryForm.pageNo state.queryForm.pageSize分页信息截取相应数据
    state.queryIcon = listData.slice(
      (state.queryForm.pageNo - 1) * state.queryForm.pageSize,
      state.queryForm.pageNo * state.queryForm.pageSize
    )
    state.total = listData.length
  }
    ...
    <el-col :span="24">
      <vab-query-form>
        <vab-query-form-top-panel>
          <el-form class="lizhi-form" inline label-width="0" @submit.prevent>
            <el-row>
              <el-col :span="16">
                <el-form-item label="">
